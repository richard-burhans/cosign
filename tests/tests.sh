...
test_run() {
	...
	*)
    ...
    printf "${bold}Test %2s - %-60s" "${test_index}" "${desc}...${clr}"
    rc=0
    eval "${cmd}" > "tmp/eval-${test_index}.$$.out"
    actual_status=${rc}
    if [ ${actual_status} -eq 2 ]; then
	echo "SKIPPED"
	tests_skipped=$((tests_skipped + 1))
	test_log < "tmp/eval-${test_index}.$$.out" > /dev/null
	echo "#### Test ${test_index} - ${desc}: SKIPPED ####" | \
		test_log >/dev/null 2>&1
    ...
    actual_out=$(headers_trim < "tmp/eval-${test_index}.$$.out" | \
		 test_log | tee "tmp/${test_index}.$$")
...
crl_create() {
    ...
    ca_password | openssl ca -config ./openssl.cnf \
			    -passin stdin \
			    -gencrl \
			    -keyfile "CA/private/cakey.pem" \
			    -cert "CA/private/cacert.pem" \
			    -out "${crl_path}/${crl_name}" > /dev/null 2>&1
    
    hash=$(openssl crl -hash -noout -in "${crl_path}/${crl_name}")
    [ -n "${hash}" ] || die "failed to hash ${crl_path}/${crl_name}!"

    (cd "${crl_path}" && ln -sf "${crl_name}" "${hash}.r0")
}
...
cert_revoke() {
    ...
    [ -f "${cert_path}" ] || \
		die "cert_revoke: ${cert_path}: invalid certificate path"

    ca_password | openssl ca -config ./openssl.cnf \
				-passin stdin \
				-revoke "${cert_path}" \
				-keyfile "CA/private/cakey.pem" \
				-cert "CA/private/cacert.pem" > /dev/null 2>&1
}
...
create_cosign_login_html_template() {
    cat > "$(pwd)/cosign/html/login.html" <<EOF
...
create_cosign_reauth_html_template() {
    cat > "$(pwd)/cosign/html/reauth.html" <<EOF
Cosign Reauth Template
...
create_cosign_error_html_template() {
    cat > "$(pwd)/cosign/html/error.html" <<EOF
...
create_cosign_expired_error_html_template() {
    cat > "$(pwd)/cosign/html/expired_error.html" <<EOF
Cosign Expired Password Template

Title: \$t
Expiration Message: \$e
Username: \$l
Required Factors: \$f
Satisfied Factors: \$d
Service: \$c
...
create_cosign_login_error_html_template() {
    cat > "$(pwd)/cosign/html/login_error.html" <<EOF
...
create_cosign_post_error_html_template() {
    cat > "$(pwd)/cosign/html/post_error.html" <<EOF
Cosign Post Error Template
EOF
}

create_cosign_looping_html_template() {
    cat > "$(pwd)/cosign/html/looping.html" <<EOF
Cosign Looping Error Template
EOF
}

create_cosign_verify_logout_html_template() {
    cat > "$(pwd)/cosign/html/verify-logout.html" <<EOF
Cosign Verify Logout Template

Title: \$t
Post-Logout Redirect URL: \$u
EOF
}

create_cosign_html_templates() {
    mkdirs
    create_cosign_login_html_template
    create_cosign_reauth_html_template
    create_cosign_error_html_template
...
create_cosign_paths() {
    dirs="cosign cosign/cache cosign/db cosign/etc cosign/html"
...
create_cosign_conf() {
    ...
    if [ -n "${cosign_conf}" ]; then
	cosign_conf="cosign/etc/${cosign_conf}"
    else
	cosign_conf="cosign/etc/cosign.conf"
    fi
    
    cat > "${cosign_conf}" <<EOF
# autogenerated by cosign's "make test" target
...
# cgi-only
set cosigntmpldir	$(pwd)/cosign/html
...
cgi_fuzz_login() {
    ...
    case "${type}" in
    inject_html)
	printf "login=alert('injection');&password=badpass\n" > "${fuzz_file}"
	"${cgi_path}" < "${fuzz_file}"
	;;
...
stderr_match_regex() {
    ...
    [ -f "${src_file}" ] || \
		die "stderr_match_regex: bad input file ${src_file}"

    [ -n "${regex}" ] || \
		die "stderr_match_regex: missing required match pattern"

    egrep -q "${regex}" "${src_file}" 2>/dev/null
    if [ $? -ne 0 ]; then
	echo "pattern not found in stderr output"
	echo "\tno lines matching \"${regex}\" in ${src_file}"
	return 1
    fi
